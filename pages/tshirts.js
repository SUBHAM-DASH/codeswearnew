...
const Tshirt = ({ products }) => {
  const tshirts = JSON.parse(products);
                  ...
                  <div key={item._id}>
                    <Link href={`/product/${item._id}`} className="lg:w-1/4 md:w-1/3 p-3 space-y-3 space-x-12 shadow-inner my-3 w-full">
                      <div className="block relative h-[270px] rounded overflow-hidden">
                        <img alt="ecommerce" className="object-cover px-6 object-center w-full h-full block" src={item.image} />
                      </div>
                      <div className="mt-4">
...
export async function getStaticProps() {
  ...
  const products = await Products.find({ category: "t-shirts" });
  return {
    props: {
